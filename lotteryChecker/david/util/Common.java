...
public class Common implements Serializable
	...
	public java.sql.Date getDateObject(String dateWithOptionalTime)
		...
		try
			...
			if (!st.hasMoreTokens())
				...
				hour=new Integer(st.nextToken()).intValue();
			if (!st.hasMoreTokens())
				return null;
			else
				minute=new Integer(st.nextToken()).intValue();
			if (!st.hasMoreTokens())
				second=0; // seconds are optional
			else
				second=new Integer(st.nextToken()).intValue();
			Calendar cal=Calendar.getInstance();
			cal.set(year, month, day, hour, minute, second);
			return new java.sql.Date(((cal.getTime().getTime())/1000)*1000); // wax
																				// the
																				// milliseconds
			}
		catch (Exception e)
	...
	public String getPage(java.net.URL url)
		...
		try
			...
			java.io.BufferedReader dis=new java.io.BufferedReader(new java.io.InputStreamReader(url.openConnection()
				.getInputStream()));
	...
	public final String normalizeDateString(String dateString, String separator) throws Exception
		...
		if (isANumber(month)) // from Date.toString()?
			...
			day=st.nextToken();
			year=st.nextToken();
			if (st.hasMoreTokens()) time=st.nextToken("X"); // may or may not be
															// present - X
															// should not be
															// there so this
															// should return the
	 ...
	 */
	public final static Enumeration extract(String original, String leftDelimiter, boolean includeLeftDelimiter,
		String rightDelimiter, boolean includeRightDelimiter)
		{
		Vector v=new Vector();
	...
	public final String normalizeTimeString(String timeString, boolean useMilitaryTime, boolean showSeconds) throws Exception
		...
		try
			...
			if (meridian!=null) if (meridian.startsWith("A"))
				meridian="AM";
			else
				meridian="PM";
			...
			return sanctify((useMilitaryTime?padString('0', 2, hour):""+hour)+":"+padString('0', 2, minute)
				+(showSeconds?":"+padString('0', 2, second):"")+" "+meridian);
			}
		catch (Exception e)
			{
			throw new Exception(bad);
	...
	public final String stripHtml(String original)
		...
		if (original!=null)
			{
			// find all of the javascript - it may have ">" and "<" in it
			...
			// eliminate all of the style information
		...
		while (original.indexOf("  ")>=0)
			original=substitute(original, "  ", " "); // collapse spaces to
														// single space
		while (original.indexOf("\n ")>=0)
			original=substitute(original, "\n ", "\n"); // remove spaces at
														// beginning of lines
